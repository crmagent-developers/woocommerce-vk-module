	...
	class VKHttpClient {
		...
		public function __construct( $connection_timeout ) {
			...
			if ( ! class_exists( 'VKTransportClientResponse' ) ) {
				include_once( __DIR__ . '/class-wc-vk-transport-client-response.php' );
			}

			if ( ! class_exists( 'VKTransportRequestException' ) ) {
				include_once( __DIR__ . '/class-wc-vk-transport-request-exception.php' );
			}
		 ...
		 *
		 * @return VKTransportClientResponse
		 *
		 * @throws VKTransportRequestException
		 */
		...
		public function sendRequest( $url, array $opts ) {
			...
			if ( $curl_error || $curl_error_code ) {
				...
				throw new \VKTransportRequestException( $error_msg );
			}
		...
		protected function parseRawResponse( $http_status, $response ) {
			...
			return new \VKTransportClientResponse( $http_status, $headers, $body );
		}
